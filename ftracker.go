...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speed := meanSpeed(action, duration) * kmhInMsec // Преобразуем скорость из км/ч в м/с
	height = height / cmInM                          // Преобразование роста в метры
	return ((walkingCaloriesWeightMultiplier*float64(weight) + (math.Pow(speed, 2)/float64(height))*walkingSpeedHeightMultiplier*float64(weight)) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	return (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * float64(weight) * float64(duration)
}
